...
class UsersRepository(
    ...
    override fun addAdmin(item: User): User? {
        var returnItem: UserDB? = null
        databaseManager.execute(logger) {
            databaseManager
                .insert(
                    "Insert into usuarios (id,nombre,date_alta,password,imageUrl,email,date_nacimiento,admin) values (?,?,?,?,?,?,?,?)",
                    item.id.toString(),
                    item.name,
                    item.createDate,
                    item.password,
    ...
    override fun deleteAll(){
        databaseManager.execute(logger) {
            databaseManager.delete("Delete from usuarios")
        }
    }
}